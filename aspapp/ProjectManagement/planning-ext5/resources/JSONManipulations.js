...
function GetUpdatedRollUpDuration(task,callback)  {
    Ajax("Post", ROOTURL + "GetStreamBasedRollUpDuration", "application/json; charset=utf-8", task, "json", callback);
...
function Ajax(Type, Url, ContentType, Data, DataType, callbk, callingMethod,showLoadIcon) {
    ...
    if (Url.indexOf("RunIDCC") >= 0 || Url.indexOf("CheckInProject") >= 0 || Url.indexOf("ReDoCC") >= 0 || Url.indexOf("CheckBufferImpact") >= 0) {
      ...
      var JsonData=Data; 
       ...
       var JsonData = JSON.stringify(Data, stl.model.Project.JSONFilter);
    }
    $.ajax({
        type: Type,
        dataType: DataType,
