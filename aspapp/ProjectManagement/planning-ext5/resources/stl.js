...
function LoadStoresWithData() {
    subtaskTypes = [
        ...
        {
            id:"5",
            name: stl.app.getColumnDisplayName('SPI_COLUMNS_AND_LABELS_SUBTASK_TYPE_STREAMS')
        }
    ];
    taskTypes = [{
        id: "snet",
...
stl.app.onTaskSelection = function(evt, sender, $task){
    stl.app.selectedTaskElement = $task;
}
...
function getHighlightMenu(evt) {
    ...
    var task = stl.app.selectedTaskElement;//stl.app.selectedTaskEl;
...
function onHighlightImmediatePredecessorsClick(item, task, evt) {
    if (task) {
        ...
        else if ($(task).parent().data("model")){
            currentTaskUid = $(task).parent().data("model").uid;
        } else  {
            if ($(task).find(".task").data("task-uid")){
                currentTaskUid = $(task).find(".task").data("task-uid")
            } else if ($(task).closest("div[data-ms-uid]")[0]){
                currentTaskUid = $(task).closest("div[data-ms-uid]")[0].getAttribute("data-ms-uid");
            } else if($(task).find(".IMS-Milestone")) {
                currentTaskUid = $(task).find(".IMS-Milestone").data("ms-uid");
            }
        }
...
stl.app.getDefaultSubtaskType = function () {
            ...
            } else if (sConfig == StreamsSubtasktypesConfigKey) {
                subtaskType = SubtaskTypesEnum.STREAMS;
            }
            return subtaskType;
        };
